...
public sealed partial class GeneticistsConsoleUniqueEnzymesView : Control
{
    ...
    private static readonly Color ColorUnknownSlot = new(230, 165, 30);
    private bool _clearMode;
    private bool _jokerMode;

    private readonly HashSet<int> _unknownSlot = new();
    ...
    private void SelectMutation(int index)
    {
        ...
        _unknownSlot.Clear();
        for (var i = 0; i < 32 && i < mutation.RevealedSequence.Length; i++)
        {
            if (mutation.RevealedSequence[i] == 'X')
            {
                _unknownSlot.Add(i);
            }
        }
    ...
    private void LoadMutationSequence(string sequence)
    {
        for (var i = 0; i < 32; i++)
        {
            ...
            button.Modulate = _unknownSlot.Contains(i)
                ? ColorUnknownSlot
                : c switch
                {
                    'A' or 'T' => ColorAT,
    ...
    public void CycleBase(int index, bool reverse = false)
    {
        ...
        var currentChar = button.Text?.Length > 0 ? char.ToUpper(button.Text[0]) : 'X';
        var currentIndex = currentChar == 'X' || !_bases.Contains(currentChar)
    ...
    private void SetBaseToX(int index)
    {
        var button = _sequencerButtons[index];
        button.Text = "X";
        button.Modulate = _unknownSlot.Contains(index) ? ColorUnknownSlot : Color.White;
    ...
    private void RevealCorrectBase(int index)
    {
        ...
        var correctBase = mutation.OriginalSequence[index];
    ...
    public void UpdateJokerButtonState(TimeSpan? cooldownEnd)
    {
        ...
        if (!cooldownEnd.HasValue || cooldownEnd.Value <= _timing.CurTime)
        {
            ...
            JokerButton.Disabled = (ParentWindow?.IsSubjectDead ?? false) || _selectedMutationId == null
                || _isSelectedMutationStored;
    ...
    public void RefreshResearchLabel()
    {
        ...
        var total = proto.ResearchPoints;
    ...
    public void RefreshMutationIcons()
    {
        ...
        for (var i = 0; i < _mutationButtons.Count; i++)
        {
            var mut = _currentMutations[i];
            var button = (UniqueEnzymeButton)_mutationButtons[i];

            var isDiscovered = ParentWindow?.DiscoveredMutationIds?.Contains(mut.Id) ?? false;
            var isBase = ParentWindow?.BaseMutationIds?.Contains(mut.Id) ?? false;
    ...
    {
        InfoNameLabel.Text = null;
        InfoDescLabel.Text = null;
        InfoInstabilityLabel.Text = null;
        InfoResearchLabel.Text = null;
    }
    ...
    private void UpdateSaveButtonState()
    {
        ...
        var isAlreadySaved = ParentWindow?.SavedMutations.Any(m => m.Id == _selectedMutationId) ?? false;
    ...
    public void OnMutationSelected(string? id, bool isStored)
    {
        ...
        var selectedIndex = _currentMutations?.FindIndex(m => m.Id == id) ?? -1;
