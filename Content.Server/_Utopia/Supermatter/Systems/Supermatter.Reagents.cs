...
{
    public void AddReagents(SupermatterComponent sm, IReadOnlyList<ReagentQuantity>? reagents)
    {
        if (reagents == null || sm.Solution is not { } solution)
            return;

        foreach (var reagent in reagents)
        {
            _solutionContainer.TryAddReagent(solution, reagent, out _);
        }
    }
    ...
    public void ProcessReagents(Entity<SupermatterComponent> sm, float frameTime)
    {
        ...
        foreach (var reaction in _reagentReactionsCache)
        {
            ...
            var ev = new SupermatterReagentReactionEvent(reaction, frameTime);
