...
public sealed partial class Emotes : ILanguageType
{
    ...
    public void Speak(EntityUid uid, string message, string name, SpeechVerbPrototype verb, ChatTransmitRange range, IEntityManager entMan, out bool success, out string resultMessage)
    {
        ...
        var coloredMessage = lang.AccentuateMessage(uid, Language, message);

        var coloredLanguageMessage = random.Pick(Replacement);
        ...
        var verbStrings = verb.SpeechVerbStrings;
        ...
        var fontSize = FontSize.HasValue ? FontSize.Value : verb.FontSize;
        var font = Font != null && Font != "" ? Font : verb.FontId;
        ...
        foreach (var (session, data) in chat.GetRecipients(uid, SharedChatSystem.VoiceRange))
        {
            EntityUid listener;
            if (session.AttachedEntity is not { Valid: true } _)
                continue;
