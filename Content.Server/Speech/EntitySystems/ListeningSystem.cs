...
public sealed class ListeningSystem : EntitySystem
{
    ...
    {
        PingListeners(ev.Source, ev.Message, ev.ObfuscatedMessage, ev.Language); // Utopia-Tweak : Language
    }

    public void PingListeners(EntityUid source, string message, string? obfuscatedMessage, LanguagePrototype? language = null) // Utopia-Tweak : Language
    {
        ...
        var ev = new ListenEvent(message, source, language); // Utopia-Tweak : Language
        var obfuscatedEv = obfuscatedMessage == null ? null : new ListenEvent(obfuscatedMessage, source, language); // Utopia-Tweak : Language
