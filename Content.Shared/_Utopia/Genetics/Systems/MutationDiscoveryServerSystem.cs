...
public sealed class SharedMutationDiscoverySystem : EntitySystem
{
    ...
    {
        if (!TryComp(consoleUid, out TransformComponent? xform) || xform.GridUid is not { } gridUid)
            return new();
    ...
    public void DeductResearchProgress(EntityUid consoleUid, string mutationId, int deductAmount)
    {
        ...
        if (tracker == null)
        {
            if (!TryComp(consoleUid, out TransformComponent? xform) || xform.GridUid is not EntityUid gridUid)
                return;
