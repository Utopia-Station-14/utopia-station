...
public abstract partial class CESharedZLevelsSystem
{
    ...
    public HashSet<EntityUid> GetTargetGrids(EntityUid parentUid)
    {
        ...
        if (!TryComp<CEZGridComponent>(parentUid, out var zgrid))
            return grids;

        var targetGroupId = zgrid.NetworkId;

        if (!TryComp(parentUid, out TransformComponent? parentXform) || parentXform.MapUid == null)
            return grids;

        if (!TryGetMapNetwork(parentXform.MapUid.Value, out var net))
            return grids;

        var validMaps = new HashSet<EntityUid>();
        foreach (var level in net.Comp.ZLevels)
        {
        ...
        var query = EntityQueryEnumerator<MapGridComponent, TransformComponent, CEZGridComponent>();
        while (query.MoveNext(out var gridUid, out var _, out var gridXform, out var linkComp))
        {
            if (linkComp.NetworkId == targetGroupId && gridXform.MapUid != null && validMaps.Contains(gridXform.MapUid.Value))
            {
                grids.Add(gridUid);
            }
        }
